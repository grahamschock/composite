...
#define CRT_CHAN_TYPE_PROTOTYPES(name, type, nslots)                                                                   \
	static inline int  crt_chan_init_##name(struct crt_chan *c) { return crt_chan_init(c, sizeof(type), nslots); } \
	static inline void crt_chan_teardown_##name(struct crt_chan *c) { crt_chan_teardown(c); }                      \
	static inline int  crt_chan_empty_##name(struct crt_chan *c) { return __crt_chan_empty(c, nslots - 1); }       \
	static inline int  crt_chan_full_##name(struct crt_chan *c) { return __crt_chan_full(c, nslots - 1); }         \
	static inline int  crt_chan_send_##name(struct crt_chan *c, void *item)                                        \
	{                                                                                                              \
		assert(pow2(nslots));                                                                                  \
		return __crt_chan_send(c, item, nslots - 1, sizeof(type));                                             \
	}                                                                                                              \
	static inline int crt_chan_recv_##name(struct crt_chan *c, void *item)                                         \
	{                                                                                                              \
		assert(pow2(nslots));                                                                                  \
		return __crt_chan_recv(c, item, nslots - 1, sizeof(type));                                             \
	}                                                                                                              \
	static inline int crt_chan_async_send_##name(struct crt_chan *c, void *item)                                   \
	{                                                                                                              \
		assert(pow2(nslots));                                                                                  \
		if (__crt_chan_produce(c, item, nslots - 1, sizeof(type))) return -EAGAIN;                             \
		return 0;                                                                                              \
	}                                                                                                              \
	static inline int crt_chan_async_recv_##name(struct crt_chan *c, void *item)                                   \
	{                                                                                                              \
		assert(pow2(nslots));                                                                                  \
